<x-app-layout>
    <div class="fixed inset-0 flex bg-gray-100" x-data="{ showModal: false, isActive: {{ $session->is_active ? 'true' : 'false' }} }">
        ...
        <main class="flex-1 overflow-y-auto h-full">
            <div class="p-6 mt-[80px]">
                                                ...
                                                <template x-for="sub in submissions" :key="sub.id">
                                                    <tr class="bg-white shadow-sm rounded-2xl overflow-hidden">
                                                        ...
                                                        <td class="px-6 py-6 rounded-e-2xl border-y border-r border-gray-100">
                                                            <form :action="'/professor/grade/' + sub.id" method="POST" class="flex flex-col gap-2">
                                                                @csrf
                ...
                <div x-show="activeTab === 'students'" x-cloak class="bg-white p-8 rounded-2xl shadow-sm border border-gray-200 animate-fade-in">
                    <div class="flex justify-between items-center mb-8">
                        <div>
                            <h2 class="font-black text-2xl text-gray-900 tracking-tight text-uppercase">Enrolled Students</h2>
                            <p class="text-sm text-gray-500 mt-1 uppercase tracking-widest text-[10px] font-bold">Monitor student presence and connection status</p>
                        </div>
                        <span class="px-4 py-2 bg-gray-100 text-[#383838] font-black rounded-xl text-xs uppercase tracking-widest border border-gray-200">
                    ...
                    <div class="overflow-hidden border border-gray-200 rounded-xl">
                        <table class="w-full text-left border-collapse">
                            <thead class="bg-gray-50 border-b border-gray-200">
                                <tr>
                                    <th class="py-4 px-6 font-black text-gray-400 uppercase text-[10px] tracking-widest">Student Name</th>
                                    <th class="py-4 px-6 font-black text-gray-400 uppercase text-[10px] tracking-widest">Email</th>
                                    <th class="py-4 px-6 font-black text-gray-400 uppercase text-[10px] tracking-widest text-center">Live Status</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100">
                                @forelse($class->students as $student)
                                    <tr class="hover:bg-gray-50/50 transition">
                                        <td class="py-4 px-6 text-sm font-bold text-gray-900 flex items-center gap-4">
                                            <div class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-[10px] font-black text-gray-500">
                                        ...
                                        <td class="py-4 px-6 text-center">
                                            @if($student->pivot->is_present)
                                                <span class="inline-flex items-center px-3 py-1 rounded-lg text-[10px] font-bold bg-green-50 text-green-700 border border-green-200 uppercase tracking-widest shadow-sm">
                                                    <span class="w-1.5 h-1.5 mr-2 bg-green-500 rounded-full animate-pulse"></span> Active
                                                </span>
                                            @else
                                                <span class="inline-flex items-center px-3 py-1 rounded-lg text-[10px] font-bold bg-gray-50 text-gray-500 border border-gray-200 uppercase tracking-widest">
                                                    Offline
                                                </span>
                                            @endif
                                        </td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="py-12 text-center">
                                            <i class="ri-group-line text-4xl text-gray-300 block mb-3"></i>
                                            <p class="text-gray-400 font-medium text-sm">No students have enrolled in this session yet.</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                ...
                <div x-show="activeTab === 'settings'" x-cloak class="bg-white p-8 rounded-2xl shadow-sm border border-gray-200 animate-fade-in">
                    <div class="mb-8">
                        <h2 class="font-black text-2xl text-gray-900 tracking-tight text-uppercase">Session Settings</h2>
                        <p class="text-sm text-gray-500 mt-1 uppercase tracking-widest text-[10px] font-bold">Configure laboratory environment controls</p>
                    </div>
                    ...
                    <form action="#" method="POST" class="max-w-3xl space-y-6">
                        ...
                        <div class="p-6 bg-gray-50 border border-gray-200 rounded-2xl">
                            <label class="flex items-center gap-2 text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">
                                <i class="ri-global-line text-lg"></i> Allowed Websites (Whitelisting)
                            </label>
                            <p class="text-xs text-gray-500 mb-4 font-medium">Specify which websites students are permitted to access via the Secure Research Browser. Separate domains with commas.</p>
                            
                            <textarea name="whitelisted_urls" class="w-full border border-gray-300 rounded-xl p-4 focus:ring-2 focus:ring-[#383838] focus:border-transparent text-sm font-mono shadow-inner outline-none transition" rows="3" placeholder="e.g. google.com, stackoverflow.com, github.com">{{ $session->whitelisted_urls ?? 'google.com' }}</textarea>
                        </div>

                        <div class="p-6 bg-red-50 border border-red-100 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div>
                                <h4 class="font-black text-red-900 text-lg">Emergency Session Stop</h4>
                                <p class="text-xs text-red-700 mt-1 font-medium">Instantly terminate the laboratory session. This will disconnect all active students and close their secure browsers.</p>
                            </div>
                            <button type="button" @click="toggleSession()" class="px-6 py-3 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 transition shadow-lg shadow-red-200 text-xs uppercase tracking-widest whitespace-nowrap">
                                <i class="ri-alert-line mr-1"></i> Force End Session
                            </button>
                        </div>

                        <div class="pt-4 flex justify-end">
                            <button type="submit" class="px-8 py-3 bg-[#383838] text-white font-bold rounded-xl hover:bg-black transition shadow-lg shadow-gray-200 text-xs uppercase tracking-widest flex items-center gap-2">
