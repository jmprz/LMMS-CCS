<x-app-layout>
    <div class="fixed inset-0 flex bg-gray-100"
            ...
            return result.sort((a, b) => {
                ...
                return 0;
                            ...
                            <div class="flex justify-between items-center mb-8 ms-4 me-4">
                                ...
                                <button type="button"
                                    @click="$dispatch('open-quiz-editor', { url: '{{ route('professor.quizzes.create', ['session_id' => $session->id]) }}' })"
                                    class="bg-[#383838] text-white px-8 py-3 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-black transition-all shadow-xl shadow-gray-200 active:scale-95">
                                    + Create Quiz
                                </button>
                            </div>
        ...
        async removeQuiz(quizId, routeUrl) {
            ...
            try {
                ...
                this.deletedQuizzes.push(quizId);
                                ...
                                @include('professor.partials.quiz-list', ['session' => $session])
                            </div>
                            <template x-teleport="body">
                                <div x-data="{
           showEditor: false,
     ...
     @message.window="
        ...
        if ($event.data === 'close-modal') close();
     " class="fixed inset-0 z-[99999] flex items-center justify-center bg-[#383838]/80 backdrop-blur-sm p-4 sm:p-6"
                                    x-transition.opacity>

                                    <div
                                        class="bg-white rounded-[2rem] shadow-2xl w-full max-w-5xl h-[90vh] flex flex-col overflow-hidden">
                                        <div
                                            class="px-8 py-5 border-b border-gray-100 flex justify-between items-center bg-white shrink-0">
                                            <h3 class="font-black text-lg text-gray-900 uppercase tracking-tight">Quiz
                                                Editor</h3>
                                            <button x-show="!isLocked" @click="showEditor = false"
                                        ...
                                        <div class="flex-grow bg-gray-50">
                                            <iframe :src="editorUrl" class="w-full h-full border-none"></iframe>
                                        </div>
                                    </div>
                                </div>
                            </template>
                                                        ...
                                                        <div
                                                            ...
                                                            <div class="px-3 py-2 border-2 border-gray-50 rounded-xl">
                                                                <span
                                                                    ...
                                                                    x-text="Math.round((attempt.score / attempt.total_points) * 100) + '%'"></span>
                                                            </div>
                                                            <div
                                                                class="px-4 py-2 bg-[#383838] text-white rounded-xl shadow-sm">
                                                                <span class="text-xs font-black tracking-wide"
                                                                    x-text="attempt.score + ' / ' + attempt.total_points"></span>
                                                            </div>
                                                        </div>
    ...
    <script>
        ...
        async function refreshQuizList() {
            ...
            const response = await fetch('{{ route('professor.quizzes.list-partial', $session->id) }}', {
                headers: { 'Accept': 'text/html' }
            });
            const html = await response.text();
            container.innerHTML = html;
            Alpine.initTree(container); // re-bind Alpine directives on the newly injected cards
        }
