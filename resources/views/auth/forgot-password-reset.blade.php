<x-guest-layout>
    <div class="min-h-screen flex flex-col items-center justify-center p-6 bg-gray-100">
        <div class="w-full max-w-md bg-white border border-gray-400 rounded-xl shadow-md p-8">
            
            <div class="mb-6 text-left">
                <h2 class="text-4xl font-black text-gray-900 mb-2">New Password</h2>
                <p class="text-gray-600 text-md">Final Step: Secure your account access</p>
            </div>

            <p class="text-sm font-medium text-gray-500 mb-4">
                Your security code was successfully confirmed. Please create a new password to finalize your recovery.
            </p>

            <div class="mb-5 bg-gray-50 border border-gray-300 rounded-xl p-4 text-xs font-semibold text-gray-600 space-y-2">
                <span class="block text-gray-900 uppercase text-[10px] tracking-wider font-black mb-0.5">Password Requirements:</span>
                <div class="flex items-center gap-2"><div class="w-1.5 h-1.5 rounded-full bg-[#383838]"></div> Must be at least 8 characters long</div>
                <div class="flex items-center gap-2"><div class="w-1.5 h-1.5 rounded-full bg-[#383838]"></div> Include at least one uppercase letter (A-Z)</div>
                <div class="flex items-center gap-2"><div class="w-1.5 h-1.5 rounded-full bg-[#383838]"></div> Include at least one number (0-9)</div>
            </div>

            <form method="POST" action="{{ route('password.update') }}" x-data="{ showPass: false, showConfirmPass: false }" class="space-y-4">
                @csrf
                ...
                <div>
                    <x-input-label for="password_confirmation" value="Confirm New Password" class="font-bold text-xs uppercase tracking-wider text-gray-600 mb-1.5" />
                    <div class="relative">
